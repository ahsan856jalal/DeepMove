...
val_lines=os.listdir('rgb_frame_for_video')
min_blob=300
max_blob=85000
dim = (640, 640) 
gmm_save_dir='gmm_file_for_video' # gmm Directory
sot_save_dir='sot_file_for_video' # SOT directory
comb_gmm_sot='comb_gmm_sot_for_video' # saving dir
temp_dir='temp_dir'
rgb_dir='rgb_frame_for_video'
...
for img_name in val_lines:
    ...
    if not os.path.exists(join(gmm_save_dir,img_name)):
        gmm_img=np.zeros(shape=(img_h,img_w))
        print('gmm file not present')
    else:
        gmm_img=cv2.imread(join(gmm_save_dir,img_name))
        gmm_img=cv2.resize(gmm_img,dim,interpolation = cv2.INTER_AREA)
    rgb_img=cv2.imread(join(rgb_dir,img_name))
    if not os.path.exists(join(sot_save_dir,img_name)):
        sot_img=np.zeros(shape=(img_h,img_w))
        print('sot file not present')
    else:
        sot_img=cv2.imread(join(sot_save_dir,img_name),0)
    ...
    if contours_gmm:
            for i, c in enumerate(contours_gmm):
                if(cv2.contourArea(c)>min_blob and cv2.contourArea(c)<max_blob):
        ...
        for x2,y2,w2,h2 in boundRect_sot:
            ...
            if(xb>xa and yb>ya):
                ...
                if(float(area_inter)/area_min>=0.5):
                    img_patch=rgb_img[ymin_sot:ymax_sot,xmin_sot:xmax_sot]
                    ...
                    if(fish_label_det!=15 and r[1]>0.6):
                        ...
                        rgb_img=cv2.rectangle(rgb_img,(xmin_sot,ymin_sot),(xmax_sot,ymax_sot),(255,12,0),2)
    ...
    if not os.path.exists(comb_gmm_sot):
        os.makedirs(comb_gmm_sot)
    f = open(join(comb_gmm_sot,img_name).split('.png')[0]+'.txt', "w")
    f.write(xml_content)
    f.close()
    cv2.imwrite(join(comb_gmm_sot,img_name),rgb_img)
